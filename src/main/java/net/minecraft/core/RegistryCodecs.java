...
public class RegistryCodecs {
   ...
   private static <T> MapCodec<RegistryCodecs.RegistryEntry<T>> withNameAndId(ResourceKey<? extends Registry<T>> var0, MapCodec<T> var1) {
      return RecordCodecBuilder.mapCodec((var2) -> {
         return var2.group(ResourceLocation.CODEC.xmap(ResourceKey.elementKey(var0), ResourceKey::location).fieldOf("name").forGetter(RegistryCodecs.RegistryEntry::key), Codec.INT.fieldOf("id").forGetter(RegistryCodecs.RegistryEntry::id), var1.forGetter(RegistryCodecs.RegistryEntry::value)).apply(var2, RegistryCodecs.RegistryEntry::new);
      });
   }
   ...
   private static <E> Decoder<Registry<E>> dataPackAwareDecoder(final ResourceKey<? extends Registry<E>> var0, final Codec<E> var1, Decoder<Map<ResourceKey<E>, E>> var2, Lifecycle var3) {
      ...
      return new Decoder<Registry<E>>() {
         public <T> DataResult<Pair<Registry<E>, T>> decode(DynamicOps<T> var1x, T var2) {
            ...
            if (var1x instanceof RegistryOps) {
               RegistryOps var4x = (RegistryOps)var1x;
   ...
   private static <T> Codec<Map<ResourceKey<T>, T>> directCodec(ResourceKey<? extends Registry<T>> var0, Codec<T> var1) {
      return Codec.unboundedMap(ResourceLocation.CODEC.xmap(ResourceKey.elementKey(var0), ResourceKey::location), var1);
   }
