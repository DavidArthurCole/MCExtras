...
public interface RegistryAccess {
   ...
   private default Stream<RegistryAccess.RegistryEntry<?>> ownedNetworkableRegistries() {
      return this.ownedRegistries().filter((var0) -> {
         return ((RegistryAccess.RegistryData)REGISTRIES.get(var0.key)).sendToClient();
      });
   }

   private static <E> DataResult<? extends Codec<E>> getNetworkCodec(ResourceKey<? extends Registry<E>> var0) {
      return (DataResult)Optional.ofNullable((RegistryAccess.RegistryData)REGISTRIES.get(var0)).map((var0x) -> {
   ...
   static RegistryAccess.Writable builtinCopy() {
      ...
      while(var2.hasNext()) {
         ...
         if (!((ResourceKey)var3.getKey()).equals(Registry.DIMENSION_TYPE_REGISTRY)) {
   ...
   static void load(RegistryAccess.Writable var0, DynamicOps<JsonElement> var1, RegistryLoader var2) {
      ...
      while(var4.hasNext()) {
         RegistryAccess.RegistryData var5 = (RegistryAccess.RegistryData)var4.next();
   ...
   static RegistryAccess readFromDisk(Dynamic<?> var0) {
      return new RegistryAccess.ImmutableRegistryAccess((Map)REGISTRIES.keySet().stream().collect(Collectors.toMap(Function.identity(), (var1) -> {
