...
public class PeriodicNotificationManager extends SimplePreparableReloadListener<Map<String, List<PeriodicNotificationManager.Notification>>> implements AutoCloseable {
   ...
   protected Map<String, List<PeriodicNotificationManager.Notification>> prepare(ResourceManager var1, ProfilerFiller var2) {
      try {
         Resource var3 = var1.getResource(this.notifications);
         ...
         try {
            ...
            try {
               var5 = (Map)CODEC.parse(JsonOps.INSTANCE, JsonParser.parseReader(var4)).result().orElseThrow();
            ...
            if (var1 >= var6.delay) {
               ...
               if (var7 != var9) {
                  this.minecraft.execute(() -> {
                     SystemToast.add(Minecraft.getInstance().getToasts(), SystemToast.SystemToastIds.PERIODIC_NOTIFICATION, new TranslatableComponent(var6.title, new Object[]{var7}), new TranslatableComponent(var6.message, new Object[]{var7}));
                  });
                  return;
               }
            }
