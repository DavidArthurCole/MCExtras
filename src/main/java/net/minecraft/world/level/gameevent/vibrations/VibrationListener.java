...
public class VibrationListener implements GameEventListener {
   ...
   private boolean isValidVibration(GameEvent var1, @Nullable Entity var2) {
      if (this.receivingEvent.isPresent()) {
         return false;
      } else if (!var1.is(GameEventTags.VIBRATIONS)) {
         return false;
      } else {
         if (var2 != null) {
            if (var1.is(GameEventTags.IGNORE_VIBRATIONS_SNEAKING) && var2.isSteppingCarefully()) {
               return false;
            }
   ...
   private void sendSignal(Level var1, GameEvent var2, BlockPos var3, BlockPos var4) {
      ...
      if (var1 instanceof ServerLevel) {
         this.receivingDistance = Mth.floor(Math.sqrt(var3.distSqr(var4)));
