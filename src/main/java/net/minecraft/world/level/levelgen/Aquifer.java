...
public interface Aquifer {
   static Aquifer create(NoiseChunk var0, ChunkPos var1, DensityFunction var2, DensityFunction var3, DensityFunction var4, DensityFunction var5, PositionalRandomFactory var6, int var7, int var8, Aquifer.FluidPicker var9) {
   ...
   public static class NoiseBasedAquifer implements Aquifer {
      ...
      NoiseBasedAquifer(NoiseChunk var1, ChunkPos var2, DensityFunction var3, DensityFunction var4, DensityFunction var5, DensityFunction var6, PositionalRandomFactory var7, int var8, int var9, Aquifer.FluidPicker var10) {
      ...
      private Aquifer.FluidStatus computeFluid(int var1, int var2, int var3) {
         ...
         int var38 = var5 + 8 - var2;
         boolean var39 = true;
         double var40 = var8 ? Mth.clampedMap((double)var38, 0.0D, 64.0D, 1.0D, 0.0D) : 0.0D;
         double var41 = Mth.clamp(this.fluidLevelFloodednessNoise.compute(new DensityFunction.SinglePointContext(var1, var2, var3)), -1.0D, 1.0D);
         double var42 = Mth.map(var40, 1.0D, 0.0D, -0.3D, 0.8D);
         if (var41 > var42) {
            return var4;
         } else {
            double var43 = Mth.map(var40, 1.0D, 0.0D, -0.8D, 0.4D);
            if (var41 <= var43) {
               return new Aquifer.FluidStatus(DimensionType.WAY_BELOW_MIN_Y, var4.fluidType);
            } else {
               boolean var44 = true;
               boolean var20 = true;
               int var21 = Math.floorDiv(var1, 16);
               int var22 = Math.floorDiv(var2, 40);
               int var23 = Math.floorDiv(var3, 16);
               int var24 = var22 * 40 + 20;
               boolean var25 = true;
               double var26 = this.fluidLevelSpreadNoise.compute(new DensityFunction.SinglePointContext(var21, var22, var23)) * 10.0D;
               int var28 = Mth.quantize(var26, 3);
               int var29 = var24 + var28;
               int var30 = Math.min(var5, var29);
               if (var29 <= -10) {
                  boolean var31 = true;
                  boolean var32 = true;
                  int var33 = Math.floorDiv(var1, 64);
                  int var34 = Math.floorDiv(var2, 40);
                  int var35 = Math.floorDiv(var3, 64);
                  double var36 = this.lavaNoise.compute(new DensityFunction.SinglePointContext(var33, var34, var35));
                  if (Math.abs(var36) > 0.3D) {
                     return new Aquifer.FluidStatus(var30, Blocks.LAVA.defaultBlockState());
                  }
               }

               return new Aquifer.FluidStatus(var30, var4.fluidType);
            }
         }
      }
   }
