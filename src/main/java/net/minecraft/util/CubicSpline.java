...
public interface CubicSpline<C> extends ToFloatFunction<C> {
      ...
      Codec var2 = RecordCodecBuilder.create((var1x) -> {
         ...
         return var1x.group(var10001, ExtraCodecs.lazyInitializedCodec(var1::getValue).fieldOf("value").forGetter(Point::value), Codec.FLOAT.fieldOf("derivative").forGetter(Point::derivative)).apply(var1x, (var0, var1xx, var2) -> {
            record Point<C>(float a, CubicSpline<C> b, float c) {
               ...
               Point(float var1, CubicSpline<C> var2, float var3) {
                  this.location = var1;
                  this.value = var2;
   ...
   public static record Constant<C>(float a) implements CubicSpline<C> {
      ...
      public CubicSpline<C> mapAll(CubicSpline.CoordinateVisitor<C> var1) {
         return this;
      }
   ...
   public static record Multipoint<C>(ToFloatFunction<C> a, float[] b, List<CubicSpline<C>> c, float[] d) implements CubicSpline<C> {
      ...
      public CubicSpline<C> mapAll(CubicSpline.CoordinateVisitor<C> var1) {
         return new CubicSpline.Multipoint(var1.visit(this.coordinate), this.locations, this.values().stream().map((var1x) -> {
